...
import { getSqlCase } from "./helpers";
...
export const getRequestBody = async (data, entityName) => {
    ...
    for (const [relatedEntityName, relationshipNames] of Object.entries(getRelatedEntities(entityName))) {
        requestData[getSqlCase(`${relatedEntityName}Id`)] =
            requestData[getSqlCase(`${relatedEntityName}Id`)] === "undefined"
                ? null
                : parseInt(requestData[getSqlCase(`${relatedEntityName}Id`)]);
    }
