...
export const normalizeDatabaseArray = (
	array = [],
	removeLastUpdated = true,
	makeIdInteger = true
) => {
...
export const normalizeDatabaseObject = (
	object = {},
	removeLastUpdated = true,
	makeIdInteger = true
) => {
	...
	if (constraints.filter) {
		...
		Object.keys(constraints.filter).forEach((attributeName) => {
			...
			Object.keys(constraints.filter[attributeName]).forEach((condition) => {
				...
				prismaConditions.where[attributeName][prismaCondition] =
					constraints.filter[attributeName][condition];
			});
		});
	}
